...
def page_mildew_detector_body():
    st.info(
        f"* The client is interested in telling whether a given cherry leaf contains powdery mildew "
        f"or not."
        )

    st.write(
        f"* You can download a set of leaves with powdery mildew and healthy leaves for live prediction. "
        f"You can download the images from [here](https://www.kaggle.com/codeinstitute/cherry-leaves)."
        )

    st.write("---")

    images_buffer = st.file_uploader('Upload image of cherry leaf. You may select more than one.',
